...
public class ReferenceParser {
	private static String c = "1. Ананьев Б. Г. О проблемах современного человекознания / \n"
...
//		String c = "Головня Л. Р. , Дегтярьова К. Л. , Попов В. В. , Тригуб О. С. ";
//		List<String> authors = getAuthors(c);
...
//		}
		...
		try {
			...
			try {
				...
				while ((line = untokenizedLineStream.read()) != null) {
					...
					buffer.append(nameSample.toString() + "\n");
	...
	public static List<SpansListWrapper> getTextSpans(String str, InputStream modelIn) throws IOException {
		try {
			...
			try {
				String line;
				List<SpansListWrapper> buffer = new ArrayList<SpansListWrapper>();
				while ((line = untokenizedLineStream.read()) != null) {
					String whitespaceTokenizerLine[] = WhitespaceTokenizer.INSTANCE.tokenize(line);
